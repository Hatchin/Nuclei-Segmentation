...
class BowlConfig(Config):
    ...
    ASPECT_RATIO = 1.5 ## Maximum aspect ratio modification when scaling
    MIN_ENLARGE = 1.5 ## Minimum enlarging of images, note that this will be randomized
    ZOOM = 1.7 ## Maximum zoom per image, note that this will be randomized
...
# LOGS_DIRS = next(os.walk(MODEL_DIR))[1]
# previous_config = '002'
# dir_value = max([int(i) for i in LOGS_DIRS if i.startswith(previous_config)])
# for dir_name in LOGS_DIRS:
#     if dir_name.startswith(previous_config) and int(dir_name) == dir_value:
#         break

## Change this with the path to the last epoch of train step 2
model_path = os.path.join(MODEL_DIR,'00220190324T1617','final.h5') 
